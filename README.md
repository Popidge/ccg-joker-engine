...
- Mirror-by-default: to further reduce start-side bias, the default behavior is to mirror each dealt hand — each unique deal is played twice with the same hands/board, once with A starting and once with B starting. This is controlled by the CLI flag `--mirror/--no-mirror` on both ccj-selfplay and ccj-gate (default: `--mirror` enabled).
